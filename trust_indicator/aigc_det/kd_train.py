...
group.add_argument('--dataset', '-d', metavar='NAME', default='imagenet',
...
group.add_argument('--teacher_model', default='resnet50', type=str, metavar='MODEL',
...
def main():
    ...
    if args.num_classes is None:
        assert hasattr(t_model, 'num_classes'), 'Model must have `num_classes` attr if not set on cmd line/config.'
        args.num_classes = t_model.num_classes

    if args.checkpoint:
        load_checkpoint(t_model, args.checkpoint, args.use_ema)
    
    t_param_count = sum([m.numel() for m in t_model.parameters()])
    _logger.info('teacher_model %s created, param count: %d' % (args.teacher_model, t_param_count))
    
    if args.test_pool:
        t_model, test_time_pool = apply_test_time_pool(t_model, data_config)

    if args.torchscript:
        torch.jit.optimized_execution(True)
        t_model = torch.jit.script(t_model)
    if args.aot_autograd:
        assert has_functorch, "functorch is needed for --aot-autograd"
        t_model = memory_efficient_fusion(t_model)
    ...
    if args.distributed:
        t_model = torch.nn.DataParallel(t_model, device_ids=list(range(2)))
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            criterionKD = IRG(w_irg_vert=0.1, w_irg_edge=0.5, w_irg_tran=0.5)
            train_metrics = train_one_epoch(
                epoch, model, t_model, loader_train, optimizer, train_loss_fn, args,
                lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                amp_autocast=amp_autocast, loss_scaler=loss_scaler, model_ema=model_ema, mixup_fn=mixup_fn, 
                criterionCls=criterionCls, criterionKD=criterionKD, lambda_kd=1.0)
...
def train_one_epoch(
        epoch, model, t_model, loader, optimizer, loss_fn, args,
        lr_scheduler=None, saver=None, output_dir=None, amp_autocast=suppress,
        loss_scaler=None, model_ema=None, mixup_fn=None, criterionCls=None, criterionKD=None, lambda_kd=0.5):
    ...
    kd_losses_m = utils.AverageMeter()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            fea_s, l1_out_s, l2_out_s, l3_out_s, l4_out_s, out_s = model(input)
            cls_loss = loss_fn(out_s, target)
            fea_t, l1_out_t,l2_out_t,l3_out_t,l4_out_t, out_t = t_model(input)  
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                fea_s, l1_out_s, l2_out_s, l3_out_s, l4_out_s, output = model(input)
